...
function initialize() {
    ...
    $("#chatform").onkeydown = function(event) {
        if(event.keyCode == 13 && !event.shiftKey) {
		   ...
		   if (message == "/help") {
			pushMessage({nick: 'SakuraBot', text: 'Commands: /help, /online, /stats, /morestats, /setTheme [theme], /ban [user], /kick [user].'})
			   ...
			   send({cmd: 'kick', nick: userToKick});
				...
				pushMessage({nick: 'SakuraBot', text: '在线的用户: ' + onlineList});
...
var COMMANDS = {
	...
	onlineSet: function(args) {
		...
		pushMessage({nick: 'SakuraBot', text: "在线的用户: " + nicks.join(", ")})
	},
	onlineAdd: function(args) {
		var nick = args.nick
		userAdd(nick)
		pushMessage({nick: 'SakuraBot', text: nick + " 加入聊天室"})
	},
	onlineRemove: function(args) {
		var nick = args.nick
		userRemove(nick)
		pushMessage({nick: 'SakuraBot', text: nick + " 离开聊天室"})
	},
}
